...
class GraniteTranscriber:
    def __init__(self, model_id: str, device: str, dtype: str, max_new_tokens: int):
    ...
    def _transcribe_sync(self, wav_path: str, language: Optional[str]) -> str:
        ...
        if lang_key in LANG_NAME:
            prompt_text = f"<|audio|>Please transcribe the speech. The spoken language is {LANG_NAME[lang_key]}."
        else:
...
class GraniteEventHandler(AsyncEventHandler):
    def __init__(self, wyoming_info: Info, transcriber: GraniteTranscriber, default_language: Optional[str], *args, **kwargs):
    ...
    async def handle_event(self, event: Event) -> bool:
        ...
        if AudioStop.is_type(event.type):
            ...
            await self.write_event(Transcript(text=text, language=norm_lang(lang)).event())
...
async def main() -> None:
    ...
    transcriber = GraniteTranscriber(args.model, args.device, args.dtype, args.max_new_tokens)
